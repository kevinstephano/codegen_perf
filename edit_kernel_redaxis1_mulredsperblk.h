...
__global__ void kernel1(Tensor<__half, 2> T0, Tensor<__half, 1> T3){
  ...
  for(size_t i95 = 0; i95 < ( ceilDiv(( ceilDiv(T0.size[1], 32) ), 4) ); ++i95 ) {
    __half* T1;
    float2 tmp;
    size_t addr = ( ( ( blockIdx.x * 16 ) + threadIdx.y ) * (T0.stride[0]/4) ) + ( ( ( ( ( i95 ) ) * 32 ) + threadIdx.x ) * T0.stride[1] );
    tmp = f2_data[addr];
    T1 = reinterpret_cast<__half*>(&tmp);
    for(size_t i96 = 0; i96 < 4; ++i96 ) {
      T4[ 0 ]
         = T4[ 0 ]
         + __half2float(T1[ i96 ]);
    }
