...
__global__ void kernel1(Tensor<__half, 2> T0, Tensor<__half, 1> T3, Tensor<float, 1> T11, Tensor<int64_t, 1> T12){
  ...
  for(size_t i119 = 0; i119 < ( ceilDiv(( ceilDiv(( ceilDiv(T0.size[0], 16) ), 256) ), 4) ); ++i119 ) {
      __half* T1;
      float2 tmp;
      size_t addr = ( ( ( ( ( ( ( i119 ) ) * 256 ) + blockIdx.y ) * 16 ) + threadIdx.y ) * T0.stride[0]/4 ) + ( ( ( blockIdx.x * 32 ) + threadIdx.x ) * T0.stride[1] );
      tmp = f2_data[addr];
      T1 = reinterpret_cast<__half*>(&tmp);
      for(size_t i120 = 0; i120 < 4; ++i120 ) {
        T4[ 0 ]
           = T4[ 0 ]
           + __half2float(T1[ i120 ]);
      }
  }
  bool T2pred;
  if ( ( ( ( blockIdx.x * 32 ) + threadIdx.x ) < T3.size[0] ) ) {
    ...
    // Allocate global tensor float T11[( ( ( ceilDiv(T3.size[0], 32) ) * 32 ) * 256 )];
    // Allocate global tensor int64_t T12[( ceilDiv(T3.size[0], 32) )];
    T2pred = reduction::gridReduce< false, true, false, true, false, true > ( T2[ 0 ], block_result, reduction_add_float, &T11[0], T12, reinterpret_cast<float*>(shared_mem));
  }
