...
__global__ void kernel1(Tensor<__half, 2> T0, Tensor<__half, 1> T3){
  ...
  for(size_t i82 = 0; i82 < ( ceilDiv(( ceilDiv(( ceilDiv(T0.size[1], 32) ), 16) ), 4) ); ++i82 ) {
      __half* T1;
      float2 tmp;
      size_t addr = ( blockIdx.x * (T0.stride[0]/4) ) + ((((i82 * 16) + threadIdx.y) * 32 ) + threadIdx.x ) * T0.stride[1];
      tmp = f2_data[addr];
      T1 = reinterpret_cast<__half*>(&tmp);
      for(size_t i83 = 0; i83 < 4; ++i83 ) {
        T4[ 0 ]
           = T4[ 0 ]
           + __half2float(T1[i83]);
      }
  }
  blockReduce< true, true, false > ( T2[ 0 ], T4[ 0 ], reduction_add_float, threadIdx, blockDim, reinterpret_cast<float*>(shared_mem));
